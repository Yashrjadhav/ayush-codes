...
Roll NUMBER(5);
CH NUMBER(1);
CURSOR cur(roll NUMBER) IS SELECT * FROM O_ROLL_CALL WHERE Roll_no 
NOT IN (SELECT Roll_no FROM N_ROLL_CALL WHERE N_ROLL_CALL.Roll_No=O_ROLL_CALL.Roll_No);
...
IF (new_var.Roll_No=Roll) THEN
 INSERT INTO N_ROLL_CALL 
VALUES(new_var.Roll_No,new_var.Name);
 CH:=1;
END IF;
END LOOP;
IF (cur%ROWCOUNT=0 OR CH=0) THEN
 DBMS_OUTPUT.PUT_LINE('RECORD PRE-EXISTS IN THE TABLE'); ELSE
